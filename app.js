...
// ─── 삭제 취소(Undo) 버퍼 ───
let _deletedItemsBuffer = []; // { item, index }[] 형태로 저장
...
function deleteChecklistRow(id) {
  ...
  if (idx === -1) return;
  // 버퍼에 저장 (되살리기 용)
  _deletedItemsBuffer = [{ item: rows[idx], index: idx }];
  state.activeProject.items = rows.filter(r => r.id !== id);
  if (state.selectedItemId === id) {
    state.selectedItemId = null; // 삭제된 항목의 선택 해제
  }
  saveActiveProject();
  renderChecklistGrid();
  showUndoToast(1); // 7초 되살리기 토스트 표시
}
...
function deleteBulkCheckedRows() {
  ...
  const ids = Array.from(state.checkedCardIds);
  const rows = state.activeProject.items || [];
  // 삭제될 항목들과 원래 인덱스를 버퍼에 저장
  _deletedItemsBuffer = [];
  ids.forEach(id => {
    const idx = rows.findIndex(r => r.id === id);
    if (idx !== -1) _deletedItemsBuffer.push({ item: rows[idx], index: idx });
  });
  _deletedItemsBuffer.sort((a, b) => a.index - b.index);
  const count = _deletedItemsBuffer.length;
  // 실제 삭제
  state.activeProject.items = rows.filter(r => !ids.includes(r.id));
  if (ids.includes(state.selectedItemId)) state.selectedItemId = null;
  // 체크 초기화
...
function showUndoToast(count) {
  ...
  toast.style.cssText = `
    ...
    font-size: 0.9rem; display: flex; align-items: center; gap: 14px;
    box-shadow: 0 8px 32px rgba(0,0,0,0.25); z-index: 99999;
  `;
  toast.innerHTML = `
    <span>🗑️ <strong>${count}개</strong> 항목이 삭제되었습니다.</span>
    <button onclick="undoDeletedItems()" style="
    ...
    ">↩ 되살리기</button>
    <div id="undo-progress" style="
      position: absolute; bottom: 0; left: 0; height: 3px;
      background: #6366f1; border-radius: 0 0 12px 12px;
      width: 100%; transition: width 7s linear;
    "></div>
  `;
...
// HTML onclick 속성에서 호출을 위한 전역 등록
